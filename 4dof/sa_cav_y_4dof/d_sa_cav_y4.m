function [x,fval,exitflag,output] = d_sa_cav_y2(fun,x0,d,s,rs,optValue,modeltemp)
...
options = saoptimset(options,'TemperatureFcn',modeltemp);
options = saoptimset(options,'ObjectiveLimit',optValue);
options = saoptimset(options,'DataType','custom');
options = saoptimset(options,'InitialTemperature',[100 100 100 100]);
options = saoptimset(options,'AnnealingFcn',@d_annealing_c4);
