...
function saida = cav_yd(vet_4_dof)
...
Tmax = max(T);
saida = [alfa1 H L t1 t0 L1 L0 Tmax];

 % Salvando imagem     
	
%         display('Salvando Figura');
%        
...
%           te=get(ht,'UserData');
%           
%           [ux,uy] = pdegrad(p,te,T); % Calculate gradient
%           ugrad = [ux;uy].*-1; % FLuxo de calor (Inverso do Gradiente)
%        %filename = 'fig';
% 	fig_filename = strcat('fig_',date,'_',  num2str(hour(now)),'-', num2str(minute(now)),'-', num2str(floor(second(now))));
%         f = figure('visible','off');
%         pdeplot(p,e,te,'xydata',T,'colormap','jet','contour','on','flowdata',ugrad);
%         saveas(f,fig_filename,'png');
fprintf('RESTRICOES:\n\npsi = %.3f \nfi = %.3f \n',psi,fi);
fprintf('\nDOFS:\n\nHL = %.3f \nt1t0 = %.3f \nL1L0 = %.3f',HdL,t1dt0,L1dL0);
fprintf('\nalpha = %.2f\n\nGEOMETRIA:\n\nH: %.6f\nL: %.6f\nt1: %.6f\nt0: %.6f\nL1: %.6f\nL0: %.6f\n\nTmax: %.8f\n\n',saida);
