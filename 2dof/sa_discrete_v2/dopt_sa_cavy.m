function opt = dopt_sa_cavy(valuet1dt0,modeltemp)    
...
%arquivo
if isempty(folder)
 foldername = strcat(num2str(i),'_sa_d_',date,'_', num2str(hour(now)),'_', num2str(minute(now)),'_', num2str(floor(second(now))),'t1dt0_',num2str(t1dt0),'_2dof_results');
else
 foldername = strcat(folder,'/',folderz,'/',folderw,'/',folderg,'/',num2str(i),'_sa_d_',date,'_',num2str(hour(now)),'_', num2str(minute(now)),'_', num2str(floor(second(now))),'t1dt0_',num2str(t1dt0),'_2dof_results');
end
mkdir(strcat('resultados/discreto/',foldername));
filename = strcat('resultados/discreto/',foldername,'/',date,'_', num2str(hour(now)),'_', num2str(minute(now)),'_', num2str(floor(second(now))),'t1dt0_',num2str(t1dt0),'_2dof_data-diary.txt');
diary (filename);
diary on
